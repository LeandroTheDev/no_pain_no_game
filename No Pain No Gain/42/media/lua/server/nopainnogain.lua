...
local muscleStrainMAX = 100;
local muscleStrainHIGH = 75;
local muscleStrainMEDIUM = 50;
local muscleStrainLOW = 25;
local muscleStrainMINIMUM = 5;
    ...
    if player:IsRunning() or player:isSprinting() then
        if player:IsRunning() then
            if enduranceLevel < 0.25 then
                player:addRightLegMuscleStrain(muscleStrainMAX);
            ...
            elseif enduranceLevel < 0.5 then
                player:addRightLegMuscleStrain(muscleStrainHIGH);
                if xpUpdate.randXp() then
                    if not isSleepy(player) then addXp(player, Perks.Fitness, 4); end
                    if debug then
            ...
            elseif enduranceLevel < 0.75 then
                player:addRightLegMuscleStrain(muscleStrainMEDIUM);
                if xpUpdate.randXp() then
        ...
        if player:getInventoryWeight() > player:getMaxWeight() * 0.5 then
            if enduranceLevel < 0.25 then
                player:addBackMuscleStrain(muscleStrainMAX);
                if xpUpdate.randXp() then
            ...
            elseif enduranceLevel < 0.5 then
                player:addBackMuscleStrain(muscleStrainHIGH);
                if xpUpdate.randXp() then
            ...
            elseif enduranceLevel < 0.75 then
                player:addBackMuscleStrain(muscleStrainMEDIUM);
                if xpUpdate.randXp() then
        ...
        elseif enduranceLevel < 0.5 then
            player:addRightLegMuscleStrain(muscleStrainLOW);
            if xpUpdate.randXp() then
                if not isSleepy(player) then addXp(player, Perks.Fitness, 2); end
                if debug then
...
local function onPlayerHit(player, weapon, hitObject, damage, hitCount)
    ...
    if not weapon:isRanged() and player:getLastHitCount() > 0 then
        if enduranceLevel < 0.25 then
            player:addCombatMuscleStrain(weapon, 1, muscleStrainHIGH * player:getLastHitCount());
        ...
        elseif enduranceLevel < 0.5 then
            player:addCombatMuscleStrain(weapon, 1, muscleStrainMEDIUM * player:getLastHitCount());
        ...
        elseif enduranceLevel < 0.75 then
            player:addCombatMuscleStrain(weapon, 1, muscleStrainLOW * player:getLastHitCount());
